...
class characterHistory {
	...
	sampleHistory(brl,startState=null){
		...
		var invState = multiDraw(["on","off"],[(1-this.model.i),this.model.i]);
...
class Model {
	...
	buildRateMatrix(){
		...
		var Adiag = -1*((this.rates[0]*this.bf[1]) + (this.rates[1]*this.bf[2]) + (this.rates[2]*this.bf[3]));
		var Cdiag = -1*((this.rates[0]*this.bf[0]) + (this.rates[3]*this.bf[2]) + (this.rates[4]*this.bf[3]));
		var Gdiag = -1*((this.rates[1]*this.bf[0]) + (this.rates[3]*this.bf[1]) + (this.rates[5]*this.bf[3]));
		var Tdiag = -1*((this.rates[2]*this.bf[0]) + (this.rates[4]*this.bf[1]) + (this.rates[5]*this.bf[2]));
		...
		return [[Adiag/norm,					  (this.rates[0]*this.bf[1])/norm, (this.rates[1]*this.bf[2])/norm, (this.rates[2]*this.bf[3])/norm],
				[(this.rates[0]*this.bf[0])/norm, Cdiag/norm, 		   			   (this.rates[3]*this.bf[2])/norm, (this.rates[4]*this.bf[3])/norm],
				[(this.rates[1]*this.bf[0])/norm, (this.rates[3]*this.bf[1])/norm, Gdiag/norm,			  			(this.rates[5]*this.bf[3])/norm],
				[(this.rates[2]*this.bf[0])/norm, (this.rates[4]*this.bf[1])/norm, (this.rates[5]*this.bf[2])/norm, Tdiag/norm]];
	}
	...
	drawNewState(currentState){
		...
		for (var i = 0; i < rateVec.length; i++){
			if (rateVec[i] >= 0){			// Ignore diagonal elements (which are < 0)
				offDiagSum += rateVec[i];
			}
		}
		for (var j = 0; j < rateVec.length; j++){
			if (rateVec[j] >= 0){			// Ignore diagonal elements (which are < 0)
